...
class EnhanceModel(BaseModel):
    ...
    def get_current_visuals(self,path, ep, iter, size=512):
        out = []
        visual_imgs = []
        ...
        save_img = np.concatenate((out_imgs[0], out_imgs[1], out_imgs[2]), axis=2)
        save_img = Image.fromarray(save_img[0])
        path1 = path+'/'+'img'
        save_path = os.path.join(path1, str(ep)+'_'+str(iter)+'_'+os.path.basename(self.path[0]))
