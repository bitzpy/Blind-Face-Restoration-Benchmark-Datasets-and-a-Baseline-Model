...
class GFPGANModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if pyramid_loss_weight > 0:
            ...
            self.output = out_rgbs[-1]
            # print("output shape : {}".format(self.output.shape))
            # for rgb in out_rgbs:
            #   print("out_rgb shape : {}".format(rgb.shape))
    ...
    def test(self):
        with torch.no_grad():
            if hasattr(self, 'net_g_ema'):
                self.net_g_ema.eval()
                self.output, out_rgbs = self.net_g_ema(self.lq)
                self.output = out_rgbs[-1]
            else:
                logger = get_root_logger()
                #logger.warning('Do not have self.net_g_ema, use self.net_g.')
                self.net_g.eval()
                self.output, out_rgbs = self.net_g(self.lq)
                self.output = out_rgbs[-1]
                self.net_g.train()
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if save_img:
                if self.opt['is_train']:
                    ...
                    save_img_path = osp.join('results/real',img_name)
                ...
                im[:,h:2*h] = sr_img
                im[:,2*h:] = gt_img
                save_img_path = save_img_path+'.png'
                imwrite(im, save_img_path)
                print(save_img_path)
            # if with_metrics:
            #     # calculate metrics
