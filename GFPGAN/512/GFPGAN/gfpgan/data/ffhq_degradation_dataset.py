...
class FFHQDegradationDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(hq_path)
        #print(lq_path)
        lq_img = cv2.cvtColor(cv2.imread(lq_path), cv2.COLOR_BGR2RGB)
       
        hq_img = cv2.cvtColor(cv2.imread(hq_path), cv2.COLOR_BGR2RGB)
        # upsampling
        # lq_img = get_512().augment_image(lq_img)
        # hq_img = get_512().augment_image(hq_img)
        ...
        lq_img = lq_img.transpose(2, 0, 1) / 255.
        lq_img = torch.from_numpy(lq_img).to(torch.float32)

        hq_img = hq_img.transpose(2, 0, 1) / 255.
        hq_img = torch.from_numpy(hq_img).to(torch.float32)


        # normalize
        normalize(hq_img, self.mean, self.std, inplace=True)
        ...
        if self.crop_components:
            ...
            return {'lq': lq_img, 'gt': hq_img, 'gt_path': hq_path, 'lq_path':lq_path, 'file': file}
